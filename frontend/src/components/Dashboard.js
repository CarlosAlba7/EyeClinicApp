...
import { patientAPI, appointmentAPI, employeeAPI, invoiceAPI } from '../services/api';
...
const Dashboard = ({ user }) => {
  ...
  const fetchStats = async () => {
    try {
      ...
      const [patients, appointments, todayAppts, employees, invoices] = await Promise.all([
        patientAPI.getAll(),
    ...
    <div className="container">
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          <a href="/patients" className="btn btn-primary">View Patients</a>
          <a href="/appointments" className="btn btn-success">Manage Appointments</a>
          {(user.employeeType === 'Admin' || user.employeeType === 'Receptionist') && (
            <a href="/invoices" className="btn btn-warning">View Invoices</a>
          )}
